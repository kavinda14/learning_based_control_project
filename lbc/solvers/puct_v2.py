...
class PUCT_V2(Solver):
    ...
    def __init__(self, policy_oracle=None, value_oracle=None, search_depth=10, number_simulations=1000,
                 C_pw=2.0, alpha_pw=0.5, C_exp=1.0, alpha_exp=0.25, beta_policy=0., beta_value=0., vis_on=False):
    ...
    def search(self, problem: Problem, root_state, turn=0):
        ...
        for t in range(self.number_simulations):
            ...
            for d, node in enumerate(path):
                node.total_value += self.calc_value(rewards, d, self.search_depth + 1, problem.gamma,
                                                    problem.num_robots)
                node.num_visits += 1
