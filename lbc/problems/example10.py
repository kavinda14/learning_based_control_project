...
class Example10(Problem):
    ...
    def step(self, s, a, dt):
        ...
        Bd = self.Bc * dt
        for robot in range(self.num_robots):
            s_tp1[self.state_idxs[robot], :] = np.dot(Fd, s[self.state_idxs[robot], :]) + np.dot(Bd, a[self.action_idxs[
                                                                                                           robot], :])
        return s_tp1
    ...
    def render(self, states=None, fig=None, ax=None):
        ...
        if states is not None:
            ...
            for robot in range(self.num_robots):
                ...
                ax.plot(states[0, self.state_idxs[robot][0]], states[0, self.state_idxs[robot][1]], 'o',
                        color=colors[robot])
                ax.plot(states[-1, self.state_idxs[robot][0]], states[-1, self.state_idxs[robot][1]], 's',
                        color=colors[robot])
    ...
    def plot_value_dataset(self, dataset, title):
        ...
        for robot in range(self.num_robots):
            fig, ax = plt.subplots()
            pcm = ax.tricontourf(state[:, self.state_idxs[robot][0]], state[:, self.state_idxs[robot][1]],
                                 value[:, robot])
