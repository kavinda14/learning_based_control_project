...
class Example6(Problem):
    ...
    def reward(self, s, a):
        ...
        reward[0, 0] = -1 * (np.dot((s - self.s_des).T, np.dot(self.Q, (s - self.s_des))) +
                             np.dot(a.T, np.dot(self.Ru, a))).squeeze()
        return reward
    ...
    def plot_value_dataset(self, dataset, title):
        ...
        if encodings.shape[0] > 100:
            ...
            for robot in range(self.num_robots):
                ...
                pcm = ax[0, robot].tricontourf(encodings[:, robot_idxs[0]], encodings[:, robot_idxs[1]],
                                               target[:, robot])
